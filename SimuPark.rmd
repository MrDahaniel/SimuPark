...
```{r}
entry_df <- read.table("./datasets/crowdLevelsMagicKindom.csv", header = T)
...
```{r}
forms_df <- read.csv("./datasets/formResponses.csv")[2:10]
```
...
A partir de estos 5 cuantiles, podemos definir los valores generales de la probabilidad de escoger una atracción sobre una actividad. Siendo así, se trabajarán con un total de 5 probabilidades para el desarrollo de la simulación.
...
Empecemos determinando los tiempos de espera máximos para cada uno de los arquetipos. Siendo así, se trabajaran la columna relacionada con estos datos.

```{r}
wait_df <- forms_df[8]
```
```{r, echo=FALSE}
knitr::kable(head(wait_df))
```

```{r}
reps <- table(wait_df$TiempoEspera)
# reps[[1]]
```

```{r}
tiempos_de_espera <- c()
total_respuestas <- length(wait_df[, 1])

for (val in reps) {
    tiempos_de_espera <- c(tiempos_de_espera, val[[1]] / total_respuestas)
}

reps$porcentaje <- tiempos_de_espera
knitr::kable(reps)
```
